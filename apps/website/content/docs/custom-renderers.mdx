...
The `renderers` field on `PluginConfig` lets you register custom React components for arbitrary code fence languages. Use this for engines that are not first-class plugins (AntV, D2, Graphviz, …) — without forking Streamdown. Vega / Vega-Lite, PlantUML, and Mermaid ship as official plugins; extra SVG engines can also use `plugins.diagrams` instead of a custom React renderer.
...
Prefer the official [`@streamdown/vega`](/docs/plugins/vega) plugin for `vega` / `vega-lite` fences. The example below is the custom-renderer approach if you want `vega-embed` interactivity (tooltips, selections) instead of the shared SVG diagram path.

Install `vega`, `vega-lite`, and `vega-embed`, then create a renderer that parses the JSON spec and calls `vegaEmbed`:
