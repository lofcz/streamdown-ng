...
To hook a default copy control without replacing it, pass `{ onCopy, onError }` through `controls.*.copy`. That shape works for `code`, `mermaid`, `plantuml`, `vega`, `smiles`, `openscad`, and `table` (table `onCopy` receives the chosen `"csv" | "tsv" | "md"` format):
