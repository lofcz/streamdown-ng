...
describe.each([
  "word",
  "char",
] as const)("layout whitespace italic list (%s)", (sep) => {
  it("fades the arriving suffix of an incomplete italic list item", async () => {
    // Fork remends only the trailing block, so closing `*…*` remounts the
    // loose item. Assert the new words still enter on the shared timeline.
    const clock = vi.spyOn(performance, "now").mockReturnValue(1000);
    const config = { duration: 250, stagger: 10, sep };
    ...
    expect(words.map((word) => word.textContent).join("")).toContain(
      "are underlings."
    );
    const arrived = words.slice(-3);
